...
local function findTargetForCurrentLevel()
	...
	attackThread = task.spawn(function()
		while _G.KillAuraEnabled do
			if AttackRemote then
				local guid = findTargetForCurrentLevel()
				if guid then
					pcall(function()
...
local CR_lastStartTime      = {}   -- [raidId]=os.clock() when StartChallengeRaidMap fired
...
AgentManager.RegisterEvent(AgentManager.EventNames.GainRaidsSuccessChest, function(_data)
	...
	-- After the cooldown (measured from raid start), attempt to re-create & enter the SAME raidId again
