...
local RAID_MAP_NAME = "Map201"

--============================================================
-- Helpers (shared)
--============================================================
local function isInRaidMap()
    return workspace:FindFirstChild("Maps")
        and workspace.Maps:FindFirstChild(RAID_MAP_NAME) ~= nil
end
...
--============================================================
-- Anti-spam / join state
--============================================================
local joinInProgress = false
local lastJoinAt = 0
local JOIN_COOLDOWN = 12 -- seconds; tune if needed

local function canAttemptJoin()
    if joinInProgress then return false end
    if now() - lastJoinAt < JOIN_COOLDOWN then return false end
    if isInRaidMap() then return false end
    return true
end

local function enterCityRaid(raidId)
    if not EvEnterRaid then return end
    if not canAttemptJoin() then
        print('[EnterRaid] Blocked by debounce/cooldown/isInRaid guard.')
        return
    end
    ...
    if not ok then
        ...
        joinInProgress = false -- allow retry later
    else
        print('[EnterRaid] Join requested for raid', raidId)
        -- Safety: if server doesn’t move us into Map201, clear after a bit
        task.delay(6, function()
            if not isInRaidMap() then
...
local function onRaidUpdate(data)
    if not autoRaidEnabled or not data then return end

    -- Open
    if data.id == RAID_ID and data.action == 'OpenCityRaid' then
        if isInRaidMap() then
            print('[RaidWatcher] Already inside raid map — skipping join.')
            return
        end
        if not canAttemptJoin() then
            print('[RaidWatcher] Join attempt suppressed (debounce/cooldown).')
            return
        end

        -- ensure we’re at host map first
        if currentMapId() ~= HOST_MAP_ID then
            print('[RaidWatcher] Raid opened. Teleporting to host map...')
            teleportTo(HOST_MAP_ID)
            if not waitForArrival(HOST_MAP_ID, 15) then
                print('[RaidWatcher] Host map arrival timed out; not spamming enter.')
                return
            end
            task.wait(2)
        end

        -- double-check right before firing
        if not isInRaidMap() then
            enterCityRaid(RAID_ID)
        else
            print('[RaidWatcher] Already inside raid map after teleport.')
        end
    end

    -- Close / End
    if data.id == RAID_ID
        and (data.action == 'CloseCityRaid' or data.state == 'End' or data.isOpen == 0) then
        print('[RaidWatcher] Raid closed. Returning to lobby...')
        teleportTo(LOBBY_MAP_ID)
        -- reset join state so the next window can be joined cleanly
        joinInProgress = false
    end
end
...
local function startRaidWatcher()
    ...
    watchdogThread = task.spawn(function()
        local t0 = now()
        while autoRaidEnabled and (now() - t0) < RAID_DURATION then
            RunService.Heartbeat:Wait()
        end
        if autoRaidEnabled and currentMapId() == HOST_MAP_ID and not isInRaidMap() then
            print('[RaidWatcher] Watchdog: Raid duration expired. Teleporting to lobby...')
            teleportTo(LOBBY_MAP_ID)
            joinInProgress = false
        end
    end)
...
print('✅ Combined UI loaded: Kill Aura + Auto City Raid toggle + quick teleports.')
