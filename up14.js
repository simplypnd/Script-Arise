...
-- Up5-style global kill aura state
_G.KillAuraEnabled = _G.KillAuraEnabled or false
_G.HitboxSize      = _G.HitboxSize or 2000
...
local function fireSettingAutoDraw(flag)
    ...
    if flag then
        ...
        -- instead of turning autoDraw off, enable autoSell
        payload = {
            {
                key   = "autoSell",
                value = true,
...
local function createAndStartRaid(raidId)
    ...
    if R_StartRaid then
        pcall(function()
            ...
            -- Immediately enable autodraw + kill aura after starting raid
...
-- In-raid detection (Up5-style: flag OR RaidsManager.raidsMapInfo)
local inRaidFlag = false

local function isInRaid()
    if inRaidFlag then return true end
...
----------------------------------------------------------------
-- Raid success handler (shared by normal + challenge raids)
----------------------------------------------------------------
local function onRaidSuccess()
    if not running then return end
...
AgentManager.RegisterEvent(AgentManager.EventNames.EnterRaidsMap, function(_mapId)
    ...
    -- Ensure autodraw + kill aura are on when we load into the raid
    fireSettingAutoDraw(true)
    _G.KillAuraEnabled = true
    startAttackLoop()
end)

-- Normal raids success (personal chest)
AgentManager.RegisterEvent(AgentManager.EventNames.GainRaidsSuccessChest, function(_data)
    onRaidSuccess()
end)

-- Challenge raids success (EnchantChest spawn event – same handling)
...
---------------------------------------------------------------
-- Kill Aura toggle (Up5-style; auto-controlled by Enter/Leave)
---------------------------------------------------------------
sec:Toggle({
    Title    = "Kill Aura",
    Desc     = "Up5-style aura; auto on in raid",
