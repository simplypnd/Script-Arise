...
local function ensureOnHostMap(maxRetries)
	maxRetries = maxRetries or 3
	local attempt = 0

	while currentMapId() ~= HOST_MAP_ID and attempt < maxRetries do
		attempt += 1
		statusLabel.Text = ("Status: teleport to host (try %d/%d)"):format(attempt, maxRetries)
		pcall(function()
			EvTeleport:FireServer({ mapId = HOST_MAP_ID })
		end)
		if waitForArrival(HOST_MAP_ID, 10) then
			return true
		end
	end

	return currentMapId() == HOST_MAP_ID
end

local function tryJoinCityRaid4(maxRetries)
	maxRetries = maxRetries or 3
	local attempt = 0

	while attempt < maxRetries do
		attempt += 1
		statusLabel.Text = ("Status: join raid (try %d/%d)"):format(attempt, maxRetries)

		task.wait(1) -- small delay before each attempt
		pcall(function()
			EvEnterRaid:FireServer(CITY_RAID_ID)
		end)

		local joinDeadline = os.clock() + 10
	...
	return isInCityRaidMap()
end

----------------------------------------------------------------
-- Main: host map → check open → join with retries → kill aura → hop
----------------------------------------------------------------

task.spawn(function()
	statusLabel.Text = "Status: checking CityRaid4..."

	-- Step 1: Ensure we are on host map for CityRaid4 (with retries)
	if not ensureOnHostMap(3) then
		raidLabel.Text   = "CityRaid4: host map failed"
	...
	raidLabel.Text = "CityRaid4: OPEN (joining...)"

	-- Step 3: Join City Raid 4 (with retries)
	if not tryJoinCityRaid4(3) then
		raidLabel.Text   = "CityRaid4: join failed"
		statusLabel.Text = "Status: hopping..."
		hopToRandomServer()
		return
	end
