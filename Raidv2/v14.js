...
local function cr_isPriorityRaid(raidId)
    if not (cr_priorityRaidIds and #cr_priorityRaidIds > 0) then
        return false
    end
    for _, id in ipairs(cr_priorityRaidIds) do
        if id == raidId then
            return true
        end
    end
...
local function cr_createAndStartRaid(raidId)
    ...
    if cr_isPriorityRaid(raidId) then
        cr_useSelectedRune()
    end
...
NotifyManager.RegisterClientEvent(NotifyManager.EventData.ChallengeRaidsSuccess, function(_payload)
    ...
    if cr_isPriorityRaid(cr_lastRaidId) then
