...
task.spawn(function()
	...
	-- Step 3: Join City Raid 4
	pcall(function()
		EvEnterRaid:FireServer(CITY_RAID_ID)
	end)

	-- Wait to actually be inside the raid map
	local joinDeadline = os.clock() + 20
	while os.clock() < joinDeadline do
		if isInCityRaidMap() then break end
		RunService.Heartbeat:Wait()
	end
	if not isInCityRaidMap() then
