...
-- Track server UTC time for raid cooldowns
local lastServerUtc = 0
AgentManager.RegisterEvent(AgentManager.EventNames.ServerUTCTime, function(ts)
	lastServerUtc = ts
end)

local function getRaidCooldownRemaining(raidId)
	if not RaidsManager or not RaidsManager.raidCountdowns then return 0 end
	local endTs = RaidsManager.raidCountdowns[raidId]
	if not endTs or endTs == 0 then return 0 end
	return math.max(endTs - lastServerUtc, 0)
end

_G.KillAuraEnabled = false
_G.HitboxSize      = 2
_G.NPCFolder       = workspace:FindFirstChild("Enemys")
_G.AttackEnemyGUID = nil
...
local function getEnemiesForCurrentLevel()
	if not EnemyManager or not EnemyManager.enemyEntitys then
		return {}
	end
	...
	for guid, enemy in pairs(EnemyManager.enemyEntitys) do
		if enemy and enemy.data and enemy.data.hp and enemy.data.hp > 0 and enemy.data.enemyLevel == level then
...
local autoRaidEnabled = { [1000001]=false, [1000002]=false, [1000003]=false, [1000004]=false }
	...
	attackThread = task.spawn(function()
		while _G.KillAuraEnabled do
			if AttackRemote then
				local targets = getEnemiesForCurrentLevel()
...
AgentManager.RegisterEvent(AgentManager.EventNames.GainRaidsSuccessChest, function(_data)
	...
	-- Quit current raid to host/lobby
 	task.delay(1, function()
  	...
  	-- After the personal cooldown, attempt to re-create & enter the SAME raidId ONCE
  	task.spawn(function()
  		local deadline = os.clock() + 120 -- safety cap: don't wait forever
  		while os.clock() < deadline do
  			if not CR_running then return end
  			if getRaidCooldownRemaining(targetId) <= 0 then
  				break
  			end
  			RunService.Heartbeat:Wait()
  		end
