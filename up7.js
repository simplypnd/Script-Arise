...
local function pickRaidTargetGuid()
	...
	attackThread = task.spawn(function()
		while _G.KillAuraEnabled do
			if AttackRemote then
				if not _G.AttackEnemyGUID then
					_G.AttackEnemyGUID = pickRaidTargetGuid()
				end

				if EnemyManager and EnemyManager.enemyEntitys and _G.AttackEnemyGUID then
					local enemy = EnemyManager.enemyEntitys[_G.AttackEnemyGUID]
					if not enemy or not enemy.data or not enemy.data.hp or enemy.data.hp <= 0 then
						_G.AttackEnemyGUID = pickRaidTargetGuid()
					end
				end

				if _G.AttackEnemyGUID then
					pcall(function()
