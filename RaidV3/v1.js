...
----------------------------------------------------------------
local Extra_SelectedMonsterIds   = {} -- [monsterId] = true (explicit selection)
local Extra_SelectedGrades       = {} -- [gradeLetter] = true; empty = all
local Extra_SelectEnabled        = false

----------------------------------------------------------------
-- MonsterId -> Grade map (parse "[D]" etc. from Name)
----------------------------------------------------------------
...
local ExtraService
pcall(function()
    ExtraService = require(ReplicatedStorage.Scripts.Share.Services.ExtraService)
end)
...
if ExtraService then
    ...
    local function shouldAriseFor(info)
        ...
        -- 1) Monster filter (explicit selection by Id)
        if next(Extra_SelectedMonsterIds) then
            if not Extra_SelectedMonsterIds[info.monsterId] then
        ...
        if next(Extra_SelectedGrades) then
            ...
            if gradeLetter and not Extra_SelectedGrades[gradeLetter] then
                return false -- DESTROY
            end
        end
    ...
    ExtraService.AutoTrigger = function(guid)
        ...
        -- not decided (feature off / no info) -> original behavior
...
do
    ...
    -- Build monster dropdown values from Monster config (Id + Name)
    local monsterValues = {}
    do
        if type(MonsterConfig) == "table" then
            local tmp = {}
            for _, m in ipairs(MonsterConfig) do
                local id   = m.Id or m["Id"]
                local name = m.Name or m["Name"] or ("Monster "..tostring(id or ""))
                if id then
            ...
            for _, entry in ipairs(tmp) do
                table.insert(monsterValues, ("%s (%d)"):format(entry.label, entry.id))
            end
        end
    end
    ...
    local monsterDropdown = secE:Dropdown({
        Title     = "Monsters",
        Desc      = "Monsters eligible for ARISE (others can be DESTROYed)",
