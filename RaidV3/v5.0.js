...
-----------------------------------------------------------------
-- Stores selected monsterIds (expanded from base names in the UI)
...
local BaseNameToIds       = {} -- [baseName] = { id1, id2, ... }
        ...
        for _, m in ipairs(MonsterConfig) do
            ...
            if id and type(name) == "string" then
                ...
                if base ~= "" then
                    ...
                    BaseNameToIds[base] = BaseNameToIds[base] or {}
                    table.insert(BaseNameToIds[base], id)
                end
            end
        end
    end
...
do
    ...
    -- Label showing resolved selections (name + id)
    local selectedLabel = secE:Label({
        Title = "Selected",
        Desc  = "None",
    })
    ...
    local function refreshSelectedLabel()
        if not next(SelectedBaseNames) then
            selectedLabel:SetDesc("None")
            return
        end
        local lines = { "Selected:" }
        for base, _ in pairs(SelectedBaseNames) do
            local rows = BaseNameToRows[base]
            if rows then
                for _, row in ipairs(rows) do
                    table.insert(lines, (" - %s %d"):format(row.name, row.id))
                end
            end
        end
        selectedLabel:SetDesc(table.concat(lines, "\n"))
    end
    ...
    local monsterDropdown = secE:Dropdown({
        ...
        Callback  = function(v)
            ...
            local vals = (type(v) == "table") and v or { v }
            for _, txt in ipairs(vals) do
                local base = tostring(txt)
                if base ~= "" and base ~= "None" then
                    SelectedBaseNames[base] = true
                    local ids = BaseNameToIds[base]
                    if ids then
