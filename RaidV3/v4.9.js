...
-----------------------------------------------------------------
local Extra_SelectedMonsterIds   = {} -- [monsterBaseName] = true
...
do
    ...
    local extraToggle = secE:Toggle({
        ...
        Desc     = "Use selected monsters/grades when deciding ARISE vs DESTROY (UI only in v3)",
...
do
    ...
    if ExtraService then
        ...
        ExtraService.GenerateExtract = function(payload)
            ...
            -- Do NOT auto-trigger here; let the game call AutoTrigger as usual.
        ...
        local function shouldAriseFor(info)
            ...
            -- Try to resolve baseName and grade from config first,
            -- then fall back to the raw info.name from ExtraService.
            local baseName = MonsterIdToBaseName[info.monsterId]
            local gradeLetter = getMonsterGradeLetter(info.monsterId)
            local rawName = info.rawInfo and (info.rawInfo.name or info.rawInfo.Name)

            if (not baseName or not gradeLetter) and type(rawName) == "string" then
                -- Strip all bracket tags like [BOSS], [Monarch], [S], etc. to get base.
                local tmpBase = rawName:gsub("%s*%[[^%]]+%]", "")
                tmpBase = tmpBase:gsub("%s+$", "")
                if not baseName or baseName == "" then
                    baseName = tmpBase
                end

                -- Re-parse grade from the last [X] in the name if needed.
                if not gradeLetter then
                    local g = rawName:match("%[(%u+)%]")
                    if g and GRADE_INDEX[g] then
                        gradeLetter = g
                    end
                end
