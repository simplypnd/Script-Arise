...
do
    ...
    -- Build monster dropdown values from Monster config (full name + Id)
    local monsterValues = {}
    local IdToDisplay   = {} -- [monsterId] = display string
    do
        if type(MonsterConfig) == "table" then
            for _, m in ipairs(MonsterConfig) do
                local id   = m.Id or m["Id"]
                local name = m.Name or m["Name"]
                if id and name then
                    local label = ("%s (%d)"):format(tostring(name), id)
                    table.insert(monsterValues, label)
    ...
    local function refreshSelectedLabel()
        ...
        for id, _ in pairs(SelectedIds) do
            local label = IdToDisplay[id] or ("Id "..tostring(id))
    ...
    local monsterDropdown = secE:Dropdown({
        ...
        AllowNone = true,
        Callback  = function(v)
            Extra_SelectedMonsterIds = {} -- reset and rebuild by Id
            SelectedIds = {}
            local vals = (type(v) == "table") and v or { v }
            for _, txt in ipairs(vals) do
