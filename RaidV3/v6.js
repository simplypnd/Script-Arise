...
local DefaultConfigName = "AuraKillv2"

-- simple flag stored via WindUI flags (for UI only)
...
-----------------------------------------------------------------
-- Extra selection state (selective arise/destroy by monster / grade)
-----------------------------------------------------------------
local Extra_SelectedMonsterIds   = {} -- [monsterId] = true
local Extra_SelectedGrades       = {} -- [gradeLetter] = true; empty = all
local Extra_SelectEnabled        = false

-----------------------------------------------------------------
-- Monster config helpers: Id -> Grade / BaseName (for Extra)
-----------------------------------------------------------------
local MonsterIdToGrade    = {}
local MonsterIdToBaseName = {}
local BaseNameToRows      = {} -- [baseName] = { { id = Id, name = Name }, ... }
    ...
    if type(MonsterConfig) == "table" then
        for _, m in ipairs(MonsterConfig) do
            ...
            if id and type(name) == "string" then
                -- Resolve grade from the last [TAG] that matches a grade letter
                local lastTag
                for tag in name:gmatch("%[(%u+)%]") do
                    if GRADE_INDEX[tag] then
                        lastTag = tag
                    end
                end
                if lastTag and GRADE_INDEX[lastTag] then
                    MonsterIdToGrade[id] = lastTag
                end

                -- Base name keeps [BOSS]/[Monarch] etc., only strips trailing grade tag
                local base = name
                if lastTag and GRADE_INDEX[lastTag] then
                    base = base:gsub("%s*%["..lastTag.."%]%s*$", "")
                end
                base = base:gsub("%s+$", "")
                MonsterIdToBaseName[id] = base

                if base ~= "" then
                    BaseNameToRows[base] = BaseNameToRows[base] or {}
                    table.insert(BaseNameToRows[base], { id = id, name = name })
                end
            end
        end
    end
...
do
    ...
    local autoLoadToggle = sec:Toggle({
        ...
        Callback = function(on)
            autoLoadFlag = on and true or false
        end,
    })
    ...
    sec:Button({
        ...
        Callback = function()
            if not ConfigManager then
                WindUI:Notify({
                    Title   = "Config",
                    Content = "ConfigManager not available; cannot save.",
                    Duration= 2,
                })
                return
            end
            Window.CurrentConfig = ConfigManager:CreateConfig(DefaultConfigName)
            local ok = pcall(function()
                return Window.CurrentConfig:Save()
            end)
            WindUI:Notify({
                Title   = "Config",
                Content = ok and "AuraKillv2 config saved." or "Failed to save AuraKillv2 config.",
                Duration= 2,
            })
        end,
    })
    ...
    sec:Button({
        ...
        Callback = function()
            if not ConfigManager then
                WindUI:Notify({
                    Title   = "Config",
                    Content = "ConfigManager not available; cannot load.",
                    Duration= 2,
                })
                return
            end
            Window.CurrentConfig = ConfigManager:CreateConfig(DefaultConfigName)
            local ok = pcall(function()
                return Window.CurrentConfig:Load()
            end)
            WindUI:Notify({
                Title   = "Config",
                Content = ok and "AuraKillv2 config loaded." or "Failed to load AuraKillv2 config.",
                Duration= 2,
            })
        end,
    })
...
-- Auto-load once at startup (if ConfigManager exists)
pcall(function()
...
----------------------------------------------------------------
-- Extra tab — selective Extra (Arise / Destroy) filters (UI only)
----------------------------------------------------------------
do
    local secE = tabExtra:Section({ Title = "Selective Extra", Opened = true })

    -- Build base-name dropdown values from Monster config
    local monsterBaseValues = {}
    do
        for base, _ in pairs(BaseNameToRows) do
            table.insert(monsterBaseValues, base)
        end
        table.sort(monsterBaseValues)
    end

    local EXTRA_GRADE_OPTIONS = { "All","E","D","C","B","A","S","SS","G","N","M" }

    -- Paragraph showing resolved selections (full name + Id)
    local selectedParagraph = secE:Paragraph({
        Title = "Selected",
        Desc  = "None",
    })

    -- Like Challenge tab: allow multi-select of "worlds" (here: monster base names)
    local SelectedBaseNames = {}      -- { baseName1, baseName2, ... }
    local SelectedRowsById  = {}      -- [id] = { id = ..., name = ... }

    local function refreshSelectedLabel()
        if not next(SelectedRowsById) then
            selectedParagraph:SetDesc("None")
            return
        end
        local lines = { "Selected:" }
        for id, row in pairs(SelectedRowsById) do
            table.insert(lines, (" - %s %d"):format(row.name, id))
        end
        selectedParagraph:SetDesc(table.concat(lines, "\n"))
    end

    -- Recompute Extra_SelectedMonsterIds and the paragraph from bases+grades
    local function recomputeExtraSelection()
        Extra_SelectedMonsterIds = {}
        SelectedRowsById = {}

        if #SelectedBaseNames == 0 then
            refreshSelectedLabel()
            return
        end

        local useGradeFilter = next(Extra_SelectedGrades) ~= nil

        for _, base in ipairs(SelectedBaseNames) do
            local rows = BaseNameToRows[base]
            if rows then
                for _, row in ipairs(rows) do
                    local id    = row.id
                    local grade = getMonsterGradeLetter(id)
                    if not useGradeFilter or (grade and Extra_SelectedGrades[grade]) then
                        Extra_SelectedMonsterIds[id] = true
                        SelectedRowsById[id] = row
                    end
                end
            end
        end
    ...
    local extraToggle = secE:Toggle({
        ...
        Desc     = "Use selected monsters/grades when deciding ARISE vs DESTROY",
        Flag     = "ExtraToggle",
    ...
    local monsterDropdown = secE:Dropdown({
        ...
        Flag      = "ExtraMonsters",
        Values    = (#monsterBaseValues > 0) and monsterBaseValues or { "None" },
        Value     = {},
        Multi     = true,   -- like Challenge worlds
        AllowNone = true,
        Callback  = function(v)
            SelectedBaseNames = {}
            local vals = (type(v) == "table") and v or { v }
            for _, txt in ipairs(vals) do
                local base = tostring(txt or "")
                if base ~= "" and base ~= "None" then
                    table.insert(SelectedBaseNames, base)
                end
            end
            recomputeExtraSelection()
        end,
    })
    ...
    local gradeDropdown = secE:Dropdown({
        ...
        Callback  = function(v)
            ...
            local useAll = false
            for _, s in ipairs(vals) do
                local letter = tostring(s)
                if letter == "All" then
                    useAll = true
                    break
                end
                if GRADE_INDEX[letter] then
                    Extra_SelectedGrades[letter] = true
                end
            end
...
-- (kept simple to avoid interfering with raids / chest)
----------------------------------------------------------------
do
    ...
    if ExtraService then
        -- Wrap GenerateExtract only to cache monsterId by guid.
        ...
        ExtraService.GenerateExtract = function(payload)
            ...
            if ok and info and info.guid then
            ...
            -- Let the game continue its normal flow.
            return oldGenerateExtract(payload)
        end

        -- Decide ARISE / DESTROY based purely on monsterId + grade.
        local function shouldAriseFor(info)
            if not Extra_SelectEnabled then
                return nil -- feature off -> fall back to original autoDraw/autoSell
            end

            local monsterId   = info.monsterId
            local gradeLetter = monsterId and getMonsterGradeLetter(monsterId) or nil

            -- 1) Monster filter (by Id)
            if next(Extra_SelectedMonsterIds) then
                if not monsterId or not Extra_SelectedMonsterIds[monsterId] then
        ...
        -- Wrap AutoTrigger so our filters take over when Extra_SelectEnabled is on.
        ...
        ExtraService.AutoTrigger = function(guid)
            ...
            if Extra_SelectEnabled and info then
                if decision == true then
                    ...
                    -- Treat nil / false as DESTROY when selective is enabled.
                    return ExtraService.SellPromptTriggeredHandle(Players.LocalPlayer, guid)
                end
            end

            -- Feature off or no info -> keep original behavior.
            if decision == true then
                return ExtraService.DrawPromptTriggeredHandle(Players.LocalPlayer, guid)
