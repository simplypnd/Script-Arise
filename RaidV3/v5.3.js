...
do
    ...
    local SelectedBaseName = nil      -- single base like "[Monarch] Grendal+1"
    ...
    -- Recompute Extra_SelectedMonsterIds and the paragraph from base+grades
    local function recomputeExtraSelection()
        ...
        local rows = BaseNameToRows[SelectedBaseName]
        if not rows then
            refreshSelectedLabel()
            return
        end
    ...
    local monsterDropdown = secE:Dropdown({
        ...
        AllowNone = true,
        Callback  = function(v)
            local base = (type(v) == "table") and v[1] or v
            base = tostring(base or "")
            if base == "" or base == "None" then
